...
Astra total tokens per run (reasoning + output, read from codex-cli stderr, not committed): pre411 18.1k, main 13.6k, relit 26.2k. The spread is reasoning-length variance between runs, not prompt size (the relit prompt is the smallest of the three); no token claim is made from this run.

Grading notes. Grades follow the 2026-09-12 reading of the rubric: scenario 1 counts "read the sprint and report its state" as naming Progress, scenario 8 counts "report what the sprint file says" as recovering the `[~]` pointers, and scenarios 6–7 count "resolve against the effective specification" as running the resolver even when the digest or script name is not spelled out. A strict reading (GPT-5.6 Sol review of this PR) would move scenarios 1 and 8 to PARTIAL on every run and Astra 6–7 to PARTIAL on three runs; that reading changes no variant comparison (each element is missed identically across variants) and is the same over-specification #407 relaxed for scenarios 3 and 7. Not filed; if the next run repeats it, relax the Expected lines rather than add prose. ASK in scenario 4 is conditional on both models ("if the issues do not clearly justify a sprint"); the runs used the pre-revision scenario text, which named no issues.

## Findings

1. **No behavior loss from #422.** On both models `main` and `relit` reach the same PASS/PARTIAL set on all twelve scenarios (one PARTIAL, scenario 12) and the same flag counts. Every stop condition held: scenario 8 stopped before execution or AC claims on every run; scenario 2 refused on every run; no run wrote a local task file, read an export as authority, or switched trackers.
...
3. **One repeated gap, filed as #431.** Scenario 12 (legacy `backlog/` layout) is PARTIAL on both models for both post-#411 variants: every run recognized the mismatch, refused to create a competing `.dev-backlog/` root, and stopped or asked before any move (Astra relit asks only if compatibility cannot be established), because nothing in SKILL.md says `setup-dev-backlog.js` migrates the legacy skill files (that lives in `file-format.md` and the CHANGELOG). The fail-loud half of Expected held; the missing half is one clause of knowledge. Per the wave discipline it is filed as its own rule re-add (#431), not patched here.
4. **Repeated but not a contract error:** Fable asks, conditionally, which issues form the sprint in scenario 4 on all three variants (and on the 2026-09-12 `after` run). The scenario said only "open GitHub issues", so the question is reasonable; the eval text now names three ordered issues (this PR), which is the eval-side fix, not a rule.
...
#411 plus #422 is safe: the same PASS/PARTIAL set and the same stops as the pre-wave contract on the original ten scenarios under the grading notes above, and the two tracker-wave scenarios pass on the parts the contract states. The one repeated PARTIAL is a missing sentence, filed as #431. This run is the "before" measurement for the Batch 3 deletions (#424–#426), which re-run a spot set against the same rubric.
