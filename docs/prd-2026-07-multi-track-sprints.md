...
- G4. **Zero migration for single-track repos.** A repo with one active sprint produces byte-identical `status.sh`/`next.sh`/doctor output to pre-change behavior.
...
- `sprint-state.js`: bump `schema_version` to `2`. No arg emits `active_sprints: [ … ]` (array) plus `active_sprint` retained as a back-compat convenience = the sole element when N==1, else `null`. `--track <slug>` emits today's single-object shape. `MULTIPLE_ACTIVE_SPRINTS` is no longer thrown for disjoint tracks; it is replaced by an `OVERLAPPING_TRACKS` error only when scopes collide.
...
Every changed script has a fast path: `if active_count == 1: <emit today's exact output>`. The multi-track code only runs when a second track exists. This is the acceptance test for Phase 1 — a single-track fixture must produce identical output before/after.
...
- **R4 — Back-compat regression in the single-track path.** *Mitigation:* G4 byte-identical fixture is a merge gate for Phase 1.
