...
const {
  ...
  hasHardFailures,
  formatReport,
} = require(path.join(SKILL_SCRIPTS, "capabilities-doctor.js"));
  ...
  it("CLI emits JSON and strict exits non-zero on hard triggers", () => {
    ...
    try {
      ...
      const jsonRun = spawnSync(
        process.execPath,
        [path.join(SKILL_SCRIPTS, "capabilities-doctor.js"), "--capabilities", capPath, "--json"],
        { encoding: "utf-8" },
      );
      ...
      const strictRun = spawnSync(
        process.execPath,
        [path.join(SKILL_SCRIPTS, "capabilities-doctor.js"), "--capabilities", capPath, "--strict"],
        { encoding: "utf-8" },
      );
