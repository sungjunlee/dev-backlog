...
- A knowledge base / wiki replacement — `CHARTER.md` is a yardstick, `_context.md` is rediscovery-prone HOW-knowledge, neither is a long-form doc store.
...
- O3 [active]    A user can answer "is this project still on track?" in under 5 minutes against a stable per-project reference axis (`CHARTER.md`) · src: user
- O4 [active]    Open-issue drift (orphan work, neglected objectives, contradictions) is detectable without manual triage · src: user
- O5 [deferred]  Automated `reassess` of `CHARTER.md` wired into `relay-merge` / sprint completion — deferred to a follow-up spec
- O6 [deferred]  `/goal` completion-condition auto-emission from `CHARTER.md` + active sprint — deferred to a follow-up spec
