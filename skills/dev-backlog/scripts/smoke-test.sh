...
OUT=$(extract_section "$TMPDIR/empty-section.md" "Goal")
assert_equals "extract: empty section" "$OUT" ""

# Missing section
OUT=$(extract_section "$TMPDIR/sections.md" "Nonexistent")
assert_equals "extract: missing section" "$OUT" ""

# Section with trailing whitespace in heading
cat > "$TMPDIR/trailing.md" << 'TEOF'
## Goal
Content here.
...
count_checkboxes "$TMPDIR/checkboxes.md"
...
count_checkboxes "$TMPDIR/empty.md"
assert_equals "count: empty total" "$CB_TOTAL" "0"
assert_equals "count: empty done" "$CB_DONE" "0"

# --- find_active_sprint tests ---

mkdir -p "$TMPDIR/sprints"
cat > "$TMPDIR/sprints/completed.md" << 'EOF'
---
status: completed
---
EOF

cat > "$TMPDIR/sprints/active.md" << 'EOF'
---
status: active
---
EOF

cat > "$TMPDIR/sprints/_context.md" << 'EOF'
status: active
EOF

OUT=$(find_active_sprint "$TMPDIR/sprints")
...
# --- No active sprint ---
rm "$TMPDIR/backlog/sprints/2026-03-test.md"
cat > "$TMPDIR/backlog/sprints/2026-02-past.md" << 'EOF'
---
status: completed
---
EOF
...
mkdir -p "$TMPDIR/backlog/tasks"
cat > "$TMPDIR/backlog/tasks/BACK-1.md" << 'EOF'
---
status: To Do
---
EOF
cat > "$TMPDIR/backlog/tasks/BACK-2.md" << 'EOF'
---
status: In Progress
---
EOF

mkdir -p "$TMPDIR/backlog/completed"
cat > "$TMPDIR/backlog/completed/BACK-0.md" << 'EOF'
done
EOF
