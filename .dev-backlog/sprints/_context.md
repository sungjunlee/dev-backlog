...
- The Work rail is `gh issue view N --json body,comments` (#443): newest `## Agent Brief` comment over body, `spec_ref:` line in the body over both, fail-closed on a failed read. `effective-task-spec.js` is parked at `v0.11.0`.
