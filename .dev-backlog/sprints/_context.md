...
- Wave 3 (v0.13.0, epic #456) re-ran the keep test with a consumer grep per surface; the shipped `skills/dev-backlog/scripts/` inventory is 2,255 lines / 11 files, and repo-only helpers (`doc-drift-check.js`, `bash-runtime.js`) live in `tests/tools/`, not in the bundle. Count files with `ls | wc -l` before quoting a number: the v0.12.0 close-out miscounted 13 as 14.
