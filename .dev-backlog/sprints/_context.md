...
- Plan refs are `#N` and nothing else. The one parser is `parseIssueRef` / `parsePlanCheckbox` in `scripts/lib.js`; `lib.sh` mirrors it as `RE_ISSUE_REF` for the shell rails. The JSON identity is `{ tracker, id, ref, issue_number }` with `tracker` from `.dev-backlog/.tracker`.
...
- Commit the active sprint file to main when opening it; an untracked sprint is invisible to delegate worktrees, which may then seed a duplicate active sprint (2026-07)
...
- Keep test for any surface (2026-09 subtraction wave, epic #420): (1) guards shared/irreversible state, (2) deterministic check the model cannot cheaply redo, (3) wire contract another tool consumes. Fails all three → delete, do not rewrite. A reviewer defending a deleted doc as a "consumed contract" must show the consumer reading the doc. Since relay was deprecated (2026-09-19) the `sprint-state` JSON has no external consumer; it stays as the fresh-session recovery rail.
...
- The four surfaces scripts still own (charter rev 19 Non-Goal "Wrapping gh", epic #440): the sprint-file contract, fail-loud shared-state guards (overlap, unmoored `[~]`, `status: completed`), the `sprint-state` JSON recovery rail, and the `triage-apply` human gate. Anything else that wraps the authority CLI is deleted, not rewritten.
...
- The backlog-triage report is written by the session (44-line SKILL.md, #433). The scripts-less variant was proven on a 15-issue fixture A/B before deletion (git history (the #433 fixture A/B, removed from the tree 2026-09-19)); repeat that shape — fixture, A render, B session report, checker on close set / protection / exclusions / anchors — before deleting any other judgment script.
...
- In-flight Plan pointer grammar (parsed by `sprint-state.js`): `→ PR #N (state)` at end of line, `[branch:name]`. Any other shape reads as unmoored and fails the live smoke assertion.
...
- #366 (GitHub resilience) closed 2026-08-22 without a matrix. #367 (fresh-session conformance cadence) runs per release tag and on rail changes; only the latest run stays under `docs/conformance/`, the rest is git history.
...
- Wave 4 (v0.14.0, epic #467): `next.sh`/`status.sh` are argument plumbing over `sprint-state.js --format text`; text and JSON come from one parser, so a "next batch" disagreement between them is impossible by construction. Text mode follows the JSON semantics (Plan-only counts, `started`-ordered portfolio, overlap exits 1). When a wrapper stops sourcing `lib.sh`, check what `lib.sh` used to overwrite: Astra caught the wrappers newly honouring a `DEFAULT_BACKLOG_DIR` env var.
- A Node test that asserts a printed path must build it with `path.join`: the Windows CI job prints `nope\sprints`, not `nope/sprints` (#468).
- Measure test-suite totals with one stated glob and keep it across records; the wave-3 and wave-4 conformance records used different globs (4,653 vs 4,604 for the same tree).
- Task authority (v0.15.0, charter rev 20): `.dev-backlog/.tracker` is one line the session reads (absent = github; `backlog`/`files`; `gitlab`); the rails are the SKILL.md Task Authority table. There is no adapter; adding an authority = one table row + one `TASK_AUTHORITIES` entry in `lib.js`, and it needs a measured consumer (GitLab is the stated exception). Scripts never call `gh`/`backlog`/`glab` for task state.
- Two parsers of one file drift: the shell guard in `sprint-close.sh` and `readTaskAuthority` in `lib.js` had to be aligned on BOM, CRLF, dangling symlinks, and "absent vs unreadable" (Astra caught both rounds). If a third reader of `.tracker` is ever needed, route it through node.
- A line budget in an issue ("≤ 15 shipped lines") makes a subagent write 150-column one-liners; state readability rules instead and record the real delta afterwards.
