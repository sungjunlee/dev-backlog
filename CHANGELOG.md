...
- **CLAUDE.md/AGENTS.md unified and modernized** — `AGENTS.md` is now a symlink to `CLAUDE.md`, and the shared guidance is rewritten to the post-0.10.0 architecture: GitHub Issues as sole task authority with no task-file mirror, sprint admission by execution complexity, multi-track disjoint scopes, and prompt-judged actions on deterministic rails. The stale mirror-centric two-layer framing is gone.
- **`docs/README.md` index simplified** — a doc is now either Living or Removed (the Historical tier is folded into the Removed table, which carries pinned release-tag links); immutable records citing old paths are explicitly declared non-drift.

### Removed

- **Shipped/concluded design records (~1,600 lines)** — the three 2026-07 PRDs (adoption hardening, autonomous execution, multi-track sprints — all shipped in 0.8.0), the concluded unattended-consumer (#238) and GitHub Projects projection (#349) pilots, the shipped Windows portability design, and the superseded 2026-05 superpowers charter-axis analysis. All remain published under the [v0.10.0](https://github.com/sungjunlee/dev-backlog/tree/v0.10.0/docs) tag; `references/github-sync.md` now cites the Projects decision through a pinned tag link.
