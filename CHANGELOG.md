...
- **GitLab adapter parked** — `gitlab-tracker.js`, its `glab` fake, tests, and mentions are removed; `TRACKER_KEYS` is `["github", "files"]`. No measured consumer existed (no `glab` on the maintainer machine, no repo pins the key); the charter's measured-consumer rule (rev 18) decides re-admission. The adapter as shipped is retrievable at [`a8ddb7d`](https://github.com/sungjunlee/dev-backlog/commit/a8ddb7d) (PR #419). Closes [#424](https://github.com/sungjunlee/dev-backlog/issues/424).
- **Reassess under the status-free charter** — `.dev-backlog/triage/2026-09-17-reassess.md` is the first reassess since rev 15: no status ladder, one contradictions list, one human-gated amendment proposal (charter rev 18, capabilities, system map) covering the tracker measured-consumer rule and the spec-axis linter removal. Amendments approved and applied 2026-09-17: charter rev 18 (configured tracker, measured-consumer rule for new adapters, two Decisions rows), capabilities (gitlab parked, `component:` a free scope string), system map. `_context.md` drops the stale "#366/#367 parked" gotcha. Closes [#421](https://github.com/sungjunlee/dev-backlog/issues/421).
...
- **Tracker-wave conformance run** — twelve-scenario fresh-session run on Claude Fable 5.1 and GPT-6 Astra across `pre411` / `main` / `relit` SKILL.md variants (`docs/conformance/2026-09-17-tracker-wave.md`): no behavior loss from #411 or #422, the `files` path passes off the hot path, one repeated gap (legacy `backlog/` migration) filed as #431. `tests/evals/dev-backlog.md` gains scenarios 11–12 and names the ordered issues in scenario 4. Closes [#423](https://github.com/sungjunlee/dev-backlog/issues/423).
- **Tracker branching off the SKILL.md hot path** — per-tracker CLI names, plan-ref grammars, create commands, and close verbs move to `skills/dev-backlog/references/adapter-ports.md` (new "Per-adapter surface" table, which also owns the "never parse or write `backlog/tasks/*.md` as a product API" rule); `SKILL.md` says "configured tracker" and points there, and `## Chosen Tracker` folds into Core Contracts as one line. Hard Constraints merge the setup-only/no-runtime-switch and fail-closed rules into one bullet (4 bullets). No rail or `Done when` semantics change. SKILL.md 181 → 160 lines, never/only/must 13 → 4. Closes [#422](https://github.com/sungjunlee/dev-backlog/issues/422).
...
- **Tracker adapter ports frozen; diagnostic export off the Backlog.md tree (BREAKING for `--legacy-export` path)** — `tracker.js` port contract is documented in `skills/dev-backlog/references/adapter-ports.md` and conformance-tested with a stub adapter. `TRACKER_KEYS` stays `["github"]` (config-only; runtime never switches). Adapter failure is fail-closed with no local-file or export fallback. `sync-pull.js --legacy-export` now writes `exports/github-issues/` (not `backlog/tasks/`, not under `.dev-backlog/`) and is not branded as Backlog.md compatibility. Closes [#413](https://github.com/sungjunlee/dev-backlog/issues/413).

- **Skill execution root is `.dev-backlog/` (BREAKING)** — sprints, `.tracker`, skill `config.yml`, and triage artifacts move out of `backlog/` so they do not collide with Backlog.md's `backlog/tasks`, `docs/`, and `config.yml`. Existing repos: move `backlog/{sprints,.tracker,config.yml,triage,triage-config.yml}` to `.dev-backlog/` (backup first); leave `backlog/tasks/`, `docs/`, and `completed/` in place. `setup-dev-backlog.js` auto-migrates when `.dev-backlog/` is absent and leftover skill files are present. See `skills/dev-backlog/references/file-format.md`. Closes [#412](https://github.com/sungjunlee/dev-backlog/issues/412).

- **Skill-lightening conformance run** — plan-level before/after on Claude Fable 5.1 and GPT-6 Astra (`docs/conformance/2026-09-12-skill-lightening.md`): same outcomes and stop conditions with a ~37% smaller hot-path prompt; the pre-wave SKILL.md had carried the eval answer key inside itself. Follow-ups filed as #407 (eval expectations). Closes [#401](https://github.com/sungjunlee/dev-backlog/issues/401).
- **`backlog/` bootstrap moved into the Plan rail** — the unconditional "if `backlog/` does not exist, run setup" line made GPT-6 Astra bootstrap `backlog/` for sprint-free single-Issue work in every run. Closes [#408](https://github.com/sungjunlee/dev-backlog/issues/408).
- **Execution Path is goal + rail + stop condition per mode** — numbered micro-steps are gone; each mode names its goal, the script that is its deterministic rail, and the unchanged `Done when` line. Mode Router keeps intent → mode only. A short `## Hard Constraints` section keeps the three rules that guard shared or irreversible state (explicit GitHub mutations, `status: completed` never flips back, no unattended `spec/*` amend). Script Resolution points at `references/scripts.md` instead of duplicating its block. SKILL.md 225 → 161 lines across the wave. Closes [#399](https://github.com/sungjunlee/dev-backlog/issues/399).
- **Each SKILL.md rule stated once** — sprint admission lives in its own section only; the active-track overlap rule is one table cell (`sprint-init.js` refuses overlap); spec-field omission points at `references/file-format.md` instead of being restated in the Mode Router, Core Contracts, the frontmatter table, and Plan steps. `never` / `only` / `must` in SKILL.md: 11 / 21 / 6 → 1 / 7 / 1. Closes [#398](https://github.com/sungjunlee/dev-backlog/issues/398).
- **Legacy-tracker and export material demoted from the dev-backlog hot path** — the `sync` mode, the `### Sync / Legacy Export` section, the `.tracker`/`config.yml` selection rules, and the tracker-inference warnings leave `SKILL.md`; `references/file-format.md` is the single home for `.tracker` and `sync-pull.js --legacy-export`, and `process.md`/`scripts.md` point there. Scripts are untouched (charter compat freeze). Closes [#397](https://github.com/sungjunlee/dev-backlog/issues/397).
...
- **Historical-retrieval shadow closed no-go** — epic [#350](https://github.com/sungjunlee/dev-backlog/issues/350) ends on amendment A1 (organic log 0/10; a 2026-08-28 go was already impossible). Arm B (live GitHub/Git/spec/sprint sources) remains the retrieval path. No compiler, no `project-memory` skill, no committed retrieval artifact. Closeout [#362](https://github.com/sungjunlee/dev-backlog/issues/362).

- **Spec axis simplified: status-free charter, living-only capabilities, externalized history** — `spec/charter.md` (rev 15) drops the `[active]→[implemented]→[validated]` proof-gate ladder, denominators, and adoption counts; Objectives become five status-free direction predicates (O10, O1, O3, O4, O5). `spec/capabilities.md` keeps three living capabilities, absorbing `backlog-sync` into `tracker-task-truth` as the legacy-export constraint; `spec/system-map.md` is compressed to the current GitHub-only shape. Retired IDs (O6–O9), all superseded Decisions rows, and moved Learnings are preserved verbatim in the new `docs/spec-history.md`. Human gate: explicit user directive 2026-08-16. Closes [#377](https://github.com/sungjunlee/dev-backlog/issues/377).
- **Spec-drift checks respect immutable history** — `objectives-check.js` and `component-lint.js` now skip `status: completed` sprints, so retired charter IDs or capability slugs referenced only by completed sprints no longer hard-fail; `objectives-check.js` also accepts status-free charter objective lines (`- O10 — …`) through explicit legacy/lean line grammars (bare prose, unknown statuses, and malformed spacing do not parse; duplicate IDs keep their first occurrence), while the legacy status vocabulary keeps parsing for existing consumer charters. Both checkers read `status:` through the new shared `sprint-status.js` helper, which handles quoted values and inline comments and treats mismatched quoting fail-safe (checked, not skipped). Part of [#377](https://github.com/sungjunlee/dev-backlog/issues/377).

- **Private consumer repos aliased in public records** — evidence docs (`o3-drill-2026-08-16.md`, `historical-retrieval-shadow.md`, `mirrorless-github-pilot.md`), the charter's O5 proof and rev-14 Decisions row, sprint/triage records, and matching GitHub issue/PR text now refer to private consumer repositories through stable `consumer-A`…`consumer-E` aliases with no private links or personal-life detail; the alias map is kept outside this public repo, and CLAUDE.md gains the standing rule. Content-level fix only — git history is deliberately left intact. PR [#376](https://github.com/sungjunlee/dev-backlog/pull/376).

- **CLAUDE.md/AGENTS.md unified and modernized** — `AGENTS.md` is now a symlink to `CLAUDE.md`, and the shared guidance is rewritten to the post-0.10.0 architecture: GitHub Issues as sole task authority with no required, runtime-read task-file mirror (the explicit `sync-pull --legacy-export` diagnostic/rollback surface remains), sprint admission by execution complexity, multi-track disjoint scopes, and prompt-judged actions on deterministic rails. The stale mirror-centric two-layer framing is gone. Commit [`a006aed`](https://github.com/sungjunlee/dev-backlog/commit/a006aed) (direct to main, no tracking issue); recorded via PR [#375](https://github.com/sungjunlee/dev-backlog/pull/375).
- **`docs/README.md` index simplified** — a doc is now either Living or Removed (the Historical tier is folded into the Removed table, which carries pinned release-tag links); immutable records citing old paths are explicitly declared non-drift. Commit [`a006aed`](https://github.com/sungjunlee/dev-backlog/commit/a006aed) / PR [#375](https://github.com/sungjunlee/dev-backlog/pull/375).

### Removed

- **Working-tree spec archive** — `docs/spec-history.md` moves to Removed. The living charter keeps the never-reuse list (O3, O5–O9) and pins full texts at [`4fea158`](https://github.com/sungjunlee/dev-backlog/blob/4fea158/spec/charter.md). Human gate: 2026-08-21 after second-start-lightening close. Closes [#385](https://github.com/sungjunlee/dev-backlog/issues/385).
- **Proof-only living docs** — `o3-drill-2026-08-16.md`, `mirrorless-github-pilot.md`, `compatibility-subtraction.md`, and `historical-retrieval-shadow.md` move to Removed with pinned `9b2160c` links. The #350 no-go stays in the charter; GitHub-native freeze stays in `authority-contract.md`. Closes [#381](https://github.com/sungjunlee/dev-backlog/issues/381).
- **Shipped/concluded design records (~1,600 lines)** — the three 2026-07 PRDs (adoption hardening, autonomous execution, multi-track sprints — all shipped in 0.8.0), the concluded unattended-consumer ([#238](https://github.com/sungjunlee/dev-backlog/issues/238)) and GitHub Projects projection ([#349](https://github.com/sungjunlee/dev-backlog/issues/349)) pilots, the shipped Windows portability design, and the superseded 2026-05 superpowers charter-axis analysis. All remain published under the [v0.10.0](https://github.com/sungjunlee/dev-backlog/tree/v0.10.0/docs) tag; `references/github-sync.md` now cites the Projects decision through a pinned tag link. Commit [`a006aed`](https://github.com/sungjunlee/dev-backlog/commit/a006aed) (direct to main, no tracking issue); recorded via PR [#375](https://github.com/sungjunlee/dev-backlog/pull/375).
