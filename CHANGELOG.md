...
- **Skill execution root is `.dev-backlog/` (BREAKING)** — sprints, `.tracker`, skill `config.yml`, and triage artifacts move out of `backlog/` so they do not collide with Backlog.md's `backlog/tasks`, `docs/`, and `config.yml`. `sync-pull.js --legacy-export` still writes `backlog/tasks/` as a diagnostic/rollback export. Existing repos: move `backlog/{sprints,.tracker,config.yml,triage,triage-config.yml}` to `.dev-backlog/` (backup first); leave `backlog/tasks/`, `docs/`, and `completed/` in place. `setup-dev-backlog.js` auto-migrates when `.dev-backlog/` is absent and leftover skill files are present. See `skills/dev-backlog/references/file-format.md`. Closes [#412](https://github.com/sungjunlee/dev-backlog/issues/412).
