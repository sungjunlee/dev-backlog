...
- opencode one-shot `opencode run` (default model) can hang with zero stdout indefinitely (45min observed on a docs edit, 2026-07-05) — second opencode failure mode after the glm-5.2 review-verdict one. Prefer sonnet subagents or relay for delegated edits until diagnosed.
