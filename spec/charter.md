...
- Wrapping the authority CLI — a script survives only as a guard on shared or irreversible state, a deterministic check a session cannot cheaply redo, or a wire contract another tool consumes (today only the `sprint-state` JSON, read by dev-relay). Task reading, spec resolution, milestone seeding, and issue listing are calls to the declared authority's CLI (`gh` by default) that the session makes itself.
