...
- Tracker abstraction in code — the task authority is one `.dev-backlog/.tracker` line the session reads; scripts stay authority-neutral (`#N` refs, sprint files) and there is no adapter layer, port, export, or per-tracker script. Supported authorities are rows of the SKILL.md Task Authority table (GitHub Issues by default; Backlog.md CLI; GitLab), each a documented set of CLI verbs plus one allow-list entry, not an adapter; a new row needs a **measured consumer** (a maintainer-run repo that pins it and uses it), with one standing exception: the GitLab row is carried unmeasured because it costs one table row and one allow-list entry, no adapter code, and it is dropped at the next reassess if no repo has pinned it. The `files` adapter, the frozen adapter ports, and the diagnostic export stay parked at tag `v0.11.0`. No multi-master sync, no SaaS connectors (Jira, Linear, Notion).
...
- Required ecosystem integrations — GitHub Projects stays optional; delegation to another agent is the session's choice, not a dev-backlog integration. No compiled project-memory / retrieval layer (#350 no-go).
- Wrapping the authority CLI — a script survives only as a guard on shared or irreversible state, a deterministic check a session cannot cheaply redo, or a wire contract another surface consumes (today only the `sprint-state` JSON, the fresh-session recovery rail behind `next.sh --json` / `status.sh --json`). Task reading, spec resolution, milestone seeding, and issue listing are calls to the declared authority's CLI (`gh` by default) that the session makes itself.
...
| 2026-09-19 | Clarification of the 2026-09-18 row: "without code" means without an adapter — the wave added 56 authority-neutral script lines | the row shipped with v0.15.0 and stays; the wording was wrong | wording of the 2026-09-18 row |
| 2026-09-19 | The relay skill series is deprecated (maintainer decision): dev-backlog names no delegate, the `[run:id]` Plan pointer and the JSON `run_id` field are deleted, and the `sprint-state` JSON stays as the fresh-session recovery rail rather than a consumer contract. `component:` naming a capability heading is a plain convention | the only consumer of the JSON and the only writer of `[run:]` pointers was relay; a rail with no reader is not a wire contract | "Relay optional" in Non-Goals; "read by dev-relay" as the JSON's keep-test justification |
