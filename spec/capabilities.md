...
- New tracker adapters beyond the selectable `github` and `files` keys (GitLab and others stay out)
...
- Never fall back from `github` to `files` or the reverse; changing trackers is an explicit `.tracker` rewrite.
